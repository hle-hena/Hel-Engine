...
/*                                                                            */
/*  Last Modified: 2026/07/25 17:21:16                                        */
/*             By: hle-hena                                                   */
/*                                                                            */
/*    -----                                                                   */
/*                                                                            */
...
Renderer::Renderer(ExecutionContext &execContext, RenderPass &&pass)
	...
	if (_pass._passStarted)
		_execContext.passIndex = RenderPass::newPass();
}
...
ExecutionContext	&Renderer::executionContext(void) const	{
	return (_execContext);
}

uint32_t	Renderer::passIndex(void) const	{
	return (_execContext.passIndex);
}

DrawCall	Renderer::drawCommand(PipelineMap *pipeline, ISystemKey) const {
	DrawCall	drawCall(this, pipeline);
	drawCall.addDynamicBinding(_execContext.globalSet, _execContext.setStride, nullptr);
	return (drawCall);
}
...
DrawCall::DrawCall(const Renderer *renderer, PipelineMap *pipeline)
	:	_pipeline(pipeline),
		...
		_execContext(&renderer->_execContext),
...
DrawCall	&DrawCall::addDynamicBinding(VkDescriptorSet set,
	...
	uint32_t	offset = alignement * _execContext->passIndex;
	if (retOffset)
		(*retOffset = offset);
